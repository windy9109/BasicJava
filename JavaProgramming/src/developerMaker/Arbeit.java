...
public class Arbeit extends Place {
	...
	Arbeit(String name, int money, int hp, int intelligent, int stress){
	...
	static void bonus( Player t ){
			...
			//확률로 보너스를 준다.(소지금+)
			if(ANum > 1 && ANum < 25) {
				bonusMoney = (int)(Math.random()*20000)+10000;
				t.money = bonusMoney;
				System.out.println("오늘은 일을 잘했다! 보너스"+bonusMoney+"원을 받았다.");
			}
					...
					int jss = Integer.parseInt(js.nextLine());
								...
								consumer = (int)(Math.random()*50)+1;
									if(t.att > consumer) {
										...
										t.stress += 10; 
										t.hp -= consumer;
										System.out.println("노력했으나 물리칠수 없었다. 체력-"+consumer+", 스트레스+10");
									}
